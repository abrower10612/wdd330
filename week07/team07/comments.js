class CommentModel {
  ...
  getComments(q = null) {
    if (q === null) {
      ...
      return this.comments.filter(post => post.name === q);
    }
  }
...
function renderCommentList(element, comments) {
  ...
  comments.forEach(post => {
    let item = docjment.createElement('li');
    item.innerHTML = `${post.name}: ${post.comment}`
...
class Comments {
  ...
  showCommentList(q = null) {
    try {
      ...
      if (q !== null) {
        document.querySelector('.addComment').getElementsByClassName.style.display = 'block';
      }
      else {
